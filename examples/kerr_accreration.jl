...
const J = 0.95
const a = J/(M*c)
const su = x2^2 + a^2 * cos(x4)^2
const delta = x2^2 - r_s * x2 + a^2
...
const epsilon = 0.0
kerr = AnalyticMetric(kerr_metric_representation, coords)
accel, u0 = SymbolicIntegratorBuilder(kerr,   epsilon, do_simplify = false)
...
integrand, t, u0s = camera_rays_generator(kerr,camera_pos,camera_veloc,camera_front,camera_up,0.012/sup_res,N_x,N_y, epsilon = epsilon)

du(x,p,t) = accel(x,t)

function kerr_flatness(integrand)
    r = integrand[1]
...
condition1(u, t, integrator) = kerr_flatness(u) || redshift(u, t, integrator)
...
data = sphere_caster.(Ref(kerr), end_vectors, end_times, epsilon = epsilon, coordinate_transform = kerr_to_mink)
